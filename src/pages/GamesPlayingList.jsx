...
const GamesPlayingList = () => {
    ...
    const [fetchUser, isUserLoading, userError] = useFetching(async () => {
        ...
        const gamesResponse = await GameService.getUserPlayingGames({userId: response.data.id, page: page, size: 10});
    ...
    const [deleteFromPlayingFetching, deleteFromPlayingLoading, deleteFromPlayingError] = useFetching(async (id) => {
        ...
        updateUser.gamesPlaying = [...updateUser.gamesPlaying.filter((game) => game !== id)];
        console.log(id)
        console.log(updateUser.gamesPlaying);
